...
fn parse_input<'a>(input: &'a String) -> HashMap<&'a str, Module> {
    ...
    for l in input.lines() {
        ...
        let kind = if module == "broadcaster" { module } else { &module[0..1] };
        let name = if module == "broadcaster" { module } else { &module[1..module.len()] };
        for d in dest.iter() {
            modules.entry(d).and_modify(|m| {
                m.inc.insert(name, 0);
            }).or_insert({
                let mut m = Module {
                    kind: "",
        ...
        modules.entry(name).and_modify(|m| {
            m.kind = kind;
            m.dest = dest.clone();
        }).or_insert(Module {
            kind,
...
fn pt1(input: &String) -> i128 {
    ...
    'outer: loop {
        ...
        while let Some(task) = tasks.pop_front() {
            ...
            match m.kind {
                "button" => {
                    tasks.extend(m.dest.clone().into_iter().map(|d| {
                        (d, 0, task.0)
                    }));
                },
                "broadcaster" => {
